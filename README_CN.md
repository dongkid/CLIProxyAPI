...
> **Dongkid 分支** — 在上游基础上进行了如下修改：
> - DeepSeek 推理兼容（空 thinking 块保留、effort 级别恢复、响应翻译中空 `reasoning_content` 回传修复）
> - 继续保留或者开发用量统计功能
> - 用量统计持久化（自动保存/加载，重启不丢失）
> - OpenCode Go 会话追踪（通过 `x-opencode-session` header，按提供商可配置）
> - OpenAI-compat 提供商的可配置会话粘滞 header 转发
